...
|----|-------|-------------|--------|
| ONB-BE-01 | Lodging status workflow | Add `status` enum column (`draft \| pending_review \| published \| rejected`) to `Lodging`, default `draft`, indexed. Migration backfills existing rows to `published`. | Pending |
...
| ONB-BE-08 | Plan Free seed | Migration seeds `lodging-free` Plan (priceInCents:0, billingInterval:'lifetime', isActive:true). Optional: add `is_default` boolean column on `Plan`. | Pending |
