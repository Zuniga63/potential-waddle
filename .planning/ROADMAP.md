...
- [ ] **Phase 1: Lodging status workflow + Plan Free seed** — Migrations: `status`, `submitted_at`, `rejection_reason` columns on Lodging (backfill legacy → published); seed `lodging-free` Plan; optional `is_default` column on Plan
